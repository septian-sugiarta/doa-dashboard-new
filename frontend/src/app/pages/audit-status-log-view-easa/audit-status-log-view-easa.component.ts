...
@Component({
  selector: 'app-audit-status-log-view-easa',
  imports: [ButtonComponent, CommonModule],
  providers: [DatePipe],
  templateUrl: './audit-status-log-view-easa.component.html',
  styleUrl: './audit-status-log-view-easa.component.css'
})
export class AuditStatusLogViewEasaComponent implements OnInit {
  ...
  loadFormData(auditStatusLog_id: number): void {
    this.auditStatusLogService.getAuditStatusLog(auditStatusLog_id).subscribe(
      (data) => {
        if (data.regulation_based === 'EASA') {
          this.data = { ...data };

          this.fetchNcrFormsData();
          this.fetchActionLogData();
        } else {
          console.error('Data AuditStatusLog tidak sesuai dengan regulation_based "EASA"');
        }
      },
      (error) => {
  ...
  fetchActionLogData(): void {
    this.actionLogService.getActionLogs().subscribe(
      (actionLogs) => {
        const easaActionLogs = actionLogs.filter(actionLog => actionLog.regulation_based === 'EASA');
        if (this.data) {
          this.data.actionLogs = easaActionLogs;
        }
      },
      (error) => {
  ...
  fetchNcrFormsData(): void {
    forkJoin([
      ...
      const easaNcrForms = ncrForms.filter(ncrForm => ncrForm.RegulationBased === 'EASA');
      if (this.data) {
        this.data.ncrForms = easaNcrForms;
      }

      if (this.data) {
        this.data.ncrReplies = ncrReplies.filter(reply =>
          easaNcrForms.some(ncrForm => ncrForm.NCR_init_ID === reply.ncrId)
        );
      }
      if (this.data) {
        this.data.ncrFollowResults = ncrFollowResults.filter(followResult =>
          easaNcrForms.some(ncrForm => ncrForm.NCR_init_ID === followResult.ncrId)
        );
      }
  ...
  public convertToPDF(): void {
    ...
    html2canvas(data, { scale: 2 }).then((canvas) => {
      ...
      while (heightLeft > 0) {
        ...
        pdf.setFontSize(10);
        pdf.text('Form No.: GMF/IM/E-003 R2', 13, pageHeight - 10);
