...
@Component({
  selector: 'app-ncr-view-dgca-follow-result',
  imports: [ButtonComponent, CommonModule],
  providers: [DatePipe],
  templateUrl: './ncr-view-dgca-follow-result.component.html',
  styleUrl: './ncr-view-dgca-follow-result.component.css'
})
export class NcrViewDgcaFollowResultComponent implements OnInit {
  data: {
  ...
  ngOnInit(): void {
    ...
    if (ncrInitId) {
      ...
      this.loadReplyData(Number(ncrInitId));
      this.loadFollowResultData(Number(ncrInitId));
    }
  }
  ...
  loadReplyData(NCR_init_ID: number): void {
    this.ncrReplyService.getNcrReply(NCR_init_ID).subscribe(
      (data) => {
        this.data = { ...this.data, ...data, ncrReplyId: data.ncrReply_id };
      },
      (error) => {
  ...
  loadFollowResultData(NCR_init_ID: number): void {
    this.ncrFollowResultService.getNcrFollowResult(NCR_init_ID).subscribe(
      (data) => {
        this.data = { ...this.data, ...data, ncrFollowResult: data.ncrFollowResult_id };
      },
      (error) => {
  ...
  getFormattedImplementationDate() {
    return this.datePipe.transform(this.data?.implementation_date, 'dd MMMM yyyy');
  }
  ...
  getFormattedIdentifiedDate() {
    return this.datePipe.transform(this.data?.Identified_Date, 'dd MMMM yyyy');
  }
  getFormattedAuditorAcceptDate() {
    return this.datePipe.transform(this.data?.Auditor_Accept_date, 'dd MMMM yyyy');
  }
  getFormattedVerifiedDate() {
    return this.datePipe.transform(this.data?.Verified_Date, 'dd MMMM yyyy');
  }
  ...
  public convertToPDF(): void {
    ...
    html2canvas(data, { scale: 2 }).then((canvas) => {
      ...
      while (heightLeft > 0) {
        ...
        pdf.setFontSize(10);
        pdf.text('Form No.: GMF/IM-002 R3', 20, pageHeight - 10);
